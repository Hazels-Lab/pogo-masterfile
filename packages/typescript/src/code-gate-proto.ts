...
export type CodeGateProtoCombatVnextCodeGate = S<
	...
	>
>;
export type CodeGateProtoIbfcLwsCodeGate = S<CodeGateProto<"IBFC_LWS_CODE_GATE", Record<string, never>>>;
export type CodeGateProtoInputTrackerCodeGate = S<
	...
	>
>;
export type CodeGateProtoIsSkuAvailableNoAppIdCodeGate = S<CodeGateProto<"IS_SKU_AVAILABLE_NO_APP_ID_CODE_GATE", Record<string, never>>>;
export type CodeGateProtoMapThemeLowPerfCodeGate = S<CodeGateProto<"MAP_THEME_LOW_PERF_CODE_GATE", Record<string, never>>>;
export type CodeGateProtoPredictChargeMoveOnSubmitRollBackCodeGate = S<
	CodeGateProto<"PREDICT_CHARGE_MOVE_ON_SUBMIT_ROLL_BACK_CODE_GATE", Record<string, never>>
>;
export type CodeGateProtoPushGatewayMultiActionCodeGate = S<CodeGateProto<"PUSH_GATEWAY_MULTI_ACTION_CODE_GATE", Record<string, never>>>;
export type CodeGateProtoStampCollectionSnapshotEnabledCodeGate = S<CodeGateProto<"STAMP_COLLECTION_SNAPSHOT_ENABLED_CODE_GATE", Record<string, never>>>;
export type CodeGateProtoTodayViewV3CodeGate = S<CodeGateProto<"TODAY_VIEW_V3_CODE_GATE", Record<string, never>>>;
export type CodeGateProtoUseExtendedSettingsForMaxScaleCodeGate = S<CodeGateProto<"USE_EXTENDED_SETTINGS_FOR_MAX_SCALE_CODE_GATE", Record<string, never>>>;
export type CodeGateProtoUseGmtSkuDataCodeGate = S<CodeGateProto<"USE_GMT_SKU_DATA_CODE_GATE", Record<string, never>>>;
